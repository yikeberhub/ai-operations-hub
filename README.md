...
| Automation  | [n8n](https://n8n.io) — Gmail polling, sending replies, Slack alerts, stats aggregation     |
...
Run every file in `supabase/migrations/` in order (0001 → 0010) against your Supabase project — either via the SQL Editor in the Supabase dashboard, or the Supabase CLI:
...
1. **Email Ingestion + Triage** — Gmail Trigger → `create_email` → `categorize_email` → branch on priority → Slack
2. **Reply email** — Webhook → Gmail send node
3. **HOT Alert / Stats Webhook** — Webhook → Slack, and Webhook → `get_email_stats` → respond with JSON
